...
BATCH_SIZE = 192 * strategy.num_replicas_in_sync  # adjust if needed
...
def make_ds(folder, augment=False):
    ...
    ds = ds.map(lambda x, y: (applications.inception_v3.preprocess_input(x), y), num_parallel_calls=tf.data.AUTOTUNE)
...
with strategy.scope():
    ...
    base_model = applications.InceptionV3(include_top=False, weights="imagenet", input_shape=(IMG_SIZE[0], IMG_SIZE[1], 3))
...
# CALLBACKS
# -------------------------------
checkpoint_path = os.path.join(OUTPUT_DIR, "best_inceptionv3.h5")
cbs = [
...
# -------------------------------
print("\n===== Stage 2: Fine-tuning InceptionV3 =====\n")
with strategy.scope():
    base_model.trainable = True
    fine_tune_at = 249  # typical fine-tune start layer for InceptionV3
...
model.save(os.path.join(OUTPUT_DIR, "final_inceptionv3.h5"))
print("✔ Model saved.")
