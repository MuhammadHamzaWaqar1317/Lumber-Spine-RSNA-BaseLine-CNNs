...
def make_ds(level, folder, augment=False):
    ...
    ds = ds.map(lambda x, y: (applications.mobilenet_v2.preprocess_input(x), y),
                num_parallel_calls=tf.data.AUTOTUNE)
...
for level in LEVELS:
    print("\n" + "="*70)
    print(f"🔥 Training MobileNetV2 for Level: {level}")
    print("="*70)

    # FINAL OUTPUT STRUCTURE
    OUTPUT_DIR = os.path.join(OUTPUT_ROOT, level, "MobileNetV2")
    ...
    with strategy.scope():
        ...
        base_model = applications.MobileNetV2(include_top=False, weights="imagenet",
    ...
    with strategy.scope():
        ...
        fine_tune_at = 100  # MobileNetV2 has 155 layers; fine-tune top 55
