...
def make_ds(level, folder, augment=False):
    ...
    ds = ds.map(lambda x, y: (applications.inception_v3.preprocess_input(x), y),
                num_parallel_calls=tf.data.AUTOTUNE)
...
for level in LEVELS:
    print("\n" + "="*70)
    print(f"🔥 Training InceptionV3 for Level: {level}")
    print("="*70)

    # OUTPUT DIRECTORY
    OUTPUT_DIR = os.path.join(OUTPUT_ROOT, level, "InceptionV3")
    ...
    class_weights = {
        i: float(w)
        for i, w in enumerate(compute_class_weight(
            "balanced",
    ...
    with strategy.scope():
        ...
        # Freeze first 200 layers for stable fine-tuning
        fine_tune_at = 200
