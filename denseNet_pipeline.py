...
def make_ds(level, folder, augment=False):
    ...
    ds = ds.map(lambda x, y: (applications.densenet.preprocess_input(x), y),
                num_parallel_calls=tf.data.AUTOTUNE)
...
for level in LEVELS:
    print("\n" + "="*70)
    print(f"🔥 Training DenseNet201 for Level: {level}")
    print("="*70)

    # FINAL OUTPUT STRUCTURE
    OUTPUT_DIR = os.path.join(OUTPUT_ROOT, level, "DenseNet201")
    ...
    with strategy.scope():
        ...
        base_model = applications.DenseNet201(include_top=False, weights="imagenet",
