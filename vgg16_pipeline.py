...
def make_ds(level, folder, augment=False):
    ...
    # VGG16 preprocessing
    ds = ds.map(lambda x, y: (applications.vgg16.preprocess_input(x), y),
                num_parallel_calls=tf.data.AUTOTUNE)
...
for level in LEVELS:
    print("\n" + "="*70)
    print(f"🔥 Training VGG16 for Level: {level}")
    print("="*70)

    # OUTPUT DIRECTORY
    OUTPUT_DIR = os.path.join(OUTPUT_ROOT, level, "VGG16")
    ...
    with strategy.scope():
        ...
        # Freeze first 10 layers (VGG16 has 13 conv layers + 5 pooling)
        fine_tune_at = 10
