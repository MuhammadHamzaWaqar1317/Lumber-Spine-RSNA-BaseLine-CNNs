...
def make_ds(level, folder, augment=False):
    ...
    # ResNet50 preprocessing (IMPORTANT!)
    ds = ds.map(lambda x, y: (applications.resnet.preprocess_input(x), y),
                num_parallel_calls=tf.data.AUTOTUNE)
...
for level in LEVELS:
    print("\n" + "="*70)
    print(f"🔥 Training ResNet50 for Level: {level}")
    print("="*70)

    # OUTPUT DIRECTORY
    OUTPUT_DIR = os.path.join(OUTPUT_ROOT, level, "ResNet50")
    ...
    with strategy.scope():
        ...
        # Freeze first 100 layers for stable fine-tuning
        fine_tune_at = 100
